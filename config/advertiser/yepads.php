...
return [
    ...
    'offer_api'           => 'http://api.pbydom.com/api-ext/v1/campaigns/',
    //'set_cookie'     =>    '',
    //'offer_api_post_json' => '',
    'set_header' => ['Authorization:Token c9525375a6fbb17a05ee5886ee161d466df056e7'],
    'creative_api'        => '',
    'advertiser_id'       => '386',
    'geo_api'             => '',
    ...
    'offer_filter'        => [    //数据过滤，return false 则跳过当前offer 可自定义过滤条件，如下所示
        'revenue' => function ($var) {
            return (float)$var[ 'campaign_payout' ] < 0 ? false : true;
        },
    ...
    'conversion_field'    => [
        'offer'          => [
            ...
            'name'            => function ($var) {
                //AE|SA
                $name = fmtOfferName($var['offer'][ 'name' ]); // . " " . $var[ 'os' ] . " [" . $var[ 'countries' ] . "]");
            ...
            'revenue'         => function ($var) {
                ...
                $payout = isset($var[ 'campaign_payout' ]) ? (float)$var[ 'campaign_payout' ] : 0;
            ...
            'destination_url' => function ($var) { //具体参数对接见广告主侧文档
                ...
                */
                //方式二，自己拼接
                return $var[ 'url' ] . "&#s2#={click_id}&#s1#={aff_id}_{source_id}";
            },
            'description'     => function ($var) {
                return $var['offer']['kpi_requirements'];
            },    //offer KPI
            'currency'        => function () {
                return 'USD';
            },
        ...
        'offer_platform' => [
            'target' => function ($var) {    //匹配offer platform
                if (strpos($var['offer']['preview_link'], 'google') != false) {
                    $platforms = "android";
                } elseif (strpos($var['offer']['preview_link'], 'itunes') != false) {
                    $platforms = "ios";
                }
        ...
        'offer_geo'      => [
            'target' => function ($var) { //匹配offer geo
                ...
                $geo = $var['offer']['tags']['countries'][0]['code'];  //get 广告主国家
                $target = [];
        ...
        'offer_cap'      => [
            ...
            'adv_cap_conversion' => function ($var) {
                if ($var['caps']['campaign_limit_daily']) {
                    $cap = $var['caps']['campaign_limit_daily'];
                } else {
                    $cap = 100;
                }
                return $cap;
    ...
    'conversion_creative' => function ($item) {
        $creative = [];
        $icon_url = isset($item[ 'thumbnails' ]) ? $item[ 'thumbnails' ] : null;   //offer icon  处理JPG,png 没问题， webp格式自己想办法。
