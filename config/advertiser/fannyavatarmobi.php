...
return [
    ...
    'offer_api'           => 'http://avatarmobi.fuseclick.com/api/v2/getOffers?key=42765289157B8072ACE03E2142B8C445&a=45&page=1&limit=100',
    //'set_cookie'     =>    '',
    //'offer_api_post_json' => '',
   // 'set_header' => '',
    ...
    'offer_list'          => function ($data) {   //接入广告主数据list
        ...
        $result = [];
        if (is_array($data['data']['content'])) {
            foreach ($data['data']['content'] as $k => $item) {
                $offer_json_data = file_get_contents('http://avatarmobi.fuseclick.com/api/v2/getOfferBanners?key=42765289157B8072ACE03E2142B8C445&a=45&offer_id='.$item['id']);
                $offer_arr_data = json_decode($offer_json_data, true);
                if ($offer_arr_data['data']) {
                    $result[] = array_merge($item, $offer_arr_data);
                }
            }
        }
        return $result;
    ...
    'conversion_field'    => [
        'offer'          => [
            ...
            'offer_approval'  => function ($var) { return 2; },     //1 Require Approval 2 Public
            ...
            'destination_url' => function ($var) { //具体参数对接见广告主侧文档
                ...
                return $var[ 'tracking_link' ] . "&aff_click_id={click_id}&sub_affid={aff_id}_{source_id}";
            },
    ...
    'conversion_creative' => function ($item) {
        $creative = [];
        $icon_url = isset($item[ 'preview_url' ]) ? $item[ 'preview_url' ] : null;   //offer icon  处理JPG,png 没问题， webp格式自己想办法。
