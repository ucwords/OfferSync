...
return [
    ...
    'offer_api'           => 'http://publisher.bothads.com/publisherapi/recommended_offers/?uid=4573&key=68363a6c911fd68eb2af772f314fa739',
    //'set_cookie'     =>    '',
    //'offer_api_post_json' => '',
    'creative_api'        => '',
    'advertiser_id'  => '359',
    ...
    'offer_filter'        => [    //数据过滤，return false 则跳过当前offer 可自定义过滤条件，如下所示
       ...
       'offer_id'    => function ($var) {
            $arr = [510127,495331,507154,459523,496459,496579,501418,510415,509057,504280,502601,505675,503804];
            if (in_array($var['oid'], $arr)) {
                return true;
            }
            return false;
        }
    ...
    'conversion_field'    => [
        'offer'          => [
            ...
            'name'            => function ($var) {
                //AE|SA
                //$name = fmtOfferName($var[ 'name' ] . " " . $var[ 'os' ] . " [" . implode(',', $var[ 'platform' ]) . "]");
            ...
            'offer_approval'  => function ($var) { return 2; },     //一般上S2S设为1, API设置2 CPA 设为2
        ...
        'offer_geo'      => [
            'target' => function ($var) { //匹配offer geo
                ...
                $geo = strtoupper($var[ 'country' ][0]);  //get 广告主国家
                $target = [];
                if ($geo == 'ALL') {
                    return $target;
                }
    ...
    'conversion_creative' => function ($item) {
        $creative = [];
        $icon_url = isset($item[ 'image' ]) ? $item[ 'image' ] : null;   //offer icon  处理JPG,png 没问题， webp格式自己想办法。
