...
return [
    ...
    'unit_testing'        => [                      //断点调试
        'api_offers'                => true,       //三方api获取情况 打印advertiser offer list
    ...
    'offer_api'           => 'http://api.nposting.com/campaign/lists?api_cd=083f08b79fcfab864d5aa4eb1373a598',
    //'set_cookie'     =>    '',
    //'offer_api_post_json' => '',
   // 'set_header' => '',
    'setUserAgent' => 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/33.0.1750',
    'creative_api'        => '',
    'advertiser_id'  => '440',
    'geo_api'             => '',
    'offer_list'          => function ($data) {   //接入广告主数据list

        return isset($data[ 'body' ]['lists']) ? $data[ 'body' ]['lists'] : null;
    },
    'pay_out_rate'        => 0.8,
    ...
    'conversion_field'    => [
        'offer'          => [
            ...
            'name'            => function ($var) {
                //AE|SA
                $name = fmtOfferName($var[ 'title' ] . " " . $var[ 'os' ] . " [" . $var[ 'target_geo' ][0] . "]");
            ...
            'offer_approval'  => function ($var) { return 2; },     //一般上S2S设为1, API设置2
            'revenue_type'    => function ($var) { return 'RPA'; },
            'revenue'         => function ($var) {
                //$payout = @$var['payout'][0]['payout'];
                $payout = isset($var[ 'point' ]) ? (float)$var[ 'point' ] : 0;
            ...
            'destination_url' => function ($var) { //具体参数对接见广告主侧文档
                ...
                */
                //方式二，自己拼接
                return $var[ 'link' ] . "?ssp_click_id={click_id}&ssp_sub_param1={aff_id}_{source_id}";
            },
            'description'     => 'kpi',    //offer KPI
    ...
    'conversion_creative' => function ($item) {
        $creative = [];
        $icon_url = isset($item[ 'thumbnail' ]) ? $item[ 'thumbnail' ] : null;   //offer icon  处理JPG,png 没问题， webp格式自己想办法。
        ...
        if (isset($item[ 'creatives' ]) && !empty($item[ 'creatives' ])) {  //offer creative
            ...
            if (is_array($creatives)) {
                foreach ($creatives as $key => $fitem) {
                    $fitem[ 0 ] = str_replace('https', 'http', $fitem['url']);
