...
return [
    ...
    'offer_api'           => 'http://mapi.affle.co/publisher.php?cId=all&key=bfc571861f1a6509340846fd217c2072',
    //'set_cookie'     =>    '',
    //'offer_api_post_json' => '',
    //'set_header' => '',
    'creative_api'        => '',
    ...
    'offer_filter'        => [    //数据过滤，return false 则跳过当前offer 可自定义过滤条件，如下所示
        'offer_id'    => function ($var) {
            $arr = [1540607791, 1540607783, 1540607600, 1540607594, 1540607584, 1540607573];
            if (!in_array($var['campaignId'], $arr)) {
                return false;
            }
            return true;
        }
    ...
    'conversion_field'    => [
        'offer'          => [
            ...
            'destination_url' => function ($var) { //具体参数对接见广告主侧文档
                // 方式一，替换广告主参数
                 $fill_vars = [
                    '{Clickid}' => '{click_id}',
                    '{GAID/IDFA}'        => '{google_aid}',
                    '{Sub_ID}'   => '{source_id}_{aff_id}',
                ];
                if ($var[ 'assetType' ] == 'ios') {
                    $fill_vars[ '{GAID/IDFA}' ] = '{ios_idfa}';
                }

                return str_replace(array_keys($fill_vars), array_values($fill_vars), $var[ 'clickTrackingUrl' ]);

                //方式二，自己拼接
                //return $var[ 'clickTrackingUrl' ] . "&dv1={click_id}&nw_sub_aff={aff_id}_{source_id}";
            },
            'description'     => function ($var) {
                return empty($var['campaignKPI ']) ? ' ' : $var['campaignKPI '];
            },    //offer KPI
            'currency'        => function() {
                return 'USD';
            },
        ...
        'offer_geo'      => [
            'target' => function ($var) { //匹配offer geo
                ...
                $geo = $var['targeting']['geoTargeting']['country'];  //get 广告主国家
                $target = [];
        ...
        'offer_cap'      => [
            ...
            'adv_cap_conversion' => function ($var) { return $var['dailyInstallCap']; },
            //'adv_cap_revenue',
    ...
    'conversion_creative' => function ($item) {
        $creative = [];
        $icon_url = isset($item[ 'creatives' ][0]['creativeURL']) ? $item[ 'creatives' ][0]['creativeURL'] : null;   //offer icon  处理JPG,png 没问题， webp格式自己想办法。
        ...
        if (isset($item[ 'creatives' ]) && !empty($item[ 'creatives' ])) {  //offer creative
            ...
            if (is_array($creatives)) {
                foreach ($creatives as $key => $fitem) {
                    $fitem[ 'creativeURL' ] = str_replace('https', 'http', $fitem['creativeURL']);
                    if (getUrlCode($fitem['creativeURL']) != 200) {
                        continue;
                    }
                    //$file_name = basename($fitem[ 0 ]);
                    $file_name = md5($fitem[ 'creativeURL']) . "." . imageTypeByUrl($fitem['creativeURL']);
                    $file_url = $fitem['creativeURL' ];
                    $creative[ 'image' ][] = [
                        'name'       => $file_name,
                        'url'        => $file_url,
                        'local_path' => CreativeStorage::save($file_name, $file_url),
