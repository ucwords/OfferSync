...
return [
    ...
    'offer_api'           => 'http://api.howdoesin.net/api/v1/get?code=0e89f66f-b838-4786-97e9-23b8505fac22&pageSize=1000',
    //'set_cookie'     =>    '',
    //'offer_api_post_json' => '',
    'creative_api'        => '',
    'advertiser_id'  =>    '373',
    ...
    'conversion_field'    => [
        'offer'          => [
            ...
            'name'            => function ($var) {
                //AE|SA
                $name = fmtOfferName($var[ 'offer_name' ] . " " . $var[ 'country' ] . " [" . $var[ 'platform' ] . "]");
            ...
            'destination_url' => function ($var) { //具体参数对接见广告主侧文档
                ...
                */
                //方式二，自己拼接
                return $var[ 'tracking_link' ] . "&aff_sub2={click_id}&aff_sub5={aff_id}_{source_id}";
            },
            'description'     => 'terms',    //offer KPI
        ...
        'offer_geo'      => [
            'target' => function ($var) { //匹配offer geo
                ...
                $geo = explode(',', $var[ 'country' ]);  //get 广告主国家
                $target = [];
    ...
    'conversion_creative' => function ($item) {
        $creative = [];
        $icon_url = isset($item[ 'banners' ][0]['url']) ? $item[ 'banners' ][0]['url'] : null;   //offer icon  处理JPG,png 没问题， webp格式自己想办法。
        if (!empty($icon_url)) {
        ...
        if (isset($item[ 'banners' ]) && !empty($item[ 'banners' ])) {  //offer creative
            $creatives = $item[ 'banners' ];
            if (is_array($creatives)) {
                foreach ($creatives as $key => $fitem) {
                    ...
                    $file_name = md5($fitem['url']);// . "." . imageTypeByUrl($fitem[ 0 ]);
                    $file_url = $fitem['url'];
                    $creative[ 'image' ][] = [
                        'name'       => $file_name,
                        'url'        => $file_url,
